package co.chu3la.legume.controller;
...
import co.chu3la.legume.exception.UnkownIdentifierException;
...
public class CustomerPanelController {
    ...
    public String verifyQRCODE(HttpServletRequest request, HttpServletResponse response, Model model) {
    	...
    	boolean isCodeMatch = qrDigit.trim().equalsIgnoreCase(qRCodeServiceImpl.getTOTPCode(user.getSecret()).trim());
    	if(isCodeMatch) {
	...
	public String customerVerfication(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		try {
			...
			if(email.equalsIgnoreCase(user.getUserId()) && isMatch) {
				...
				for(co.chu3la.legume.entities.Role role : user.getRoles()) {
...
//		user.setSecret(url)
		...
		userRole.setRole((co.chu3la.legume.entities.Role) role);
